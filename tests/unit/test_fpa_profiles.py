...
@pytest.mark.mujoco
def test_n2_fpa_profile_excludes_hip_yaw_from_recovery_joints() -> None:
    groups = _fpa_joint_groups(MujocoModel.from_robot("n2"), get_fpa_profile("n2"))

    assert len(groups.recovery) == 8
    assert len(groups.left_recovery) == 4
    assert len(groups.right_recovery) == 4
    assert not set(groups.left_recovery).intersection(groups.right_recovery)
    assert all("hip_yaw" not in name for name in groups.recovery)
