...
def test_gemx_n2_overlay_keeps_unsupported_ankle_orientation_disabled() -> None:
    dmr = get_dmr_profile("n2", source_provider="gem-x")
    fpa = get_fpa_profile("n2", source_provider="gem-x")

    assert dmr.orientation_smoothing_mode == "quaternion_continuous"
    assert dmr.ankle_orientation_mode == "none"
    assert dmr.ankle_orientation_stage == "none"
    assert not fpa.use_profile_ankle_orientation
